...
const TRANSLATE_API_URL = 'https://script.google.com/macros/s/AKfycbyZzeACcCpM2PNgoYN1jg64CTzZcXCx-xYHp9WC6yNQYdrCmyYcER-_vR8A-D2Epvok_w/exec';
const GENERIC_ERROR_MESSAGE = 'エラーが発生しました';
const REQUEST_ERROR_MESSAGE = '翻訳リクエスト中にエラーが発生しました';
const GOOGLE_SCRIPT_ERROR_SIGNATURE = '<link rel="shortcut icon" href="//ssl.gstatic.com/docs/script/images/favicon.ico"><title>Error</title>'; // エラー時に返ってくるHTMLの一部
...
async function translator(text, source, target) {
  try {
    ...
    const isGoogleScriptError = response.data.includes(GOOGLE_SCRIPT_ERROR_SIGNATURE);
